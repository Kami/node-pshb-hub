...
function Route(method, path_re, context, callback) {
    return function (req, resp) {
        ...
        if (method == '*' || method.toLowerCase() == req.method.toLowerCase()) {
            ...
            if (parts = path.match(path_re)) {
                ...
                var result = callback.apply(context, slice(arguments).concat(slice(parts, 1)));
...
clutch.route = function (urls, context, req, res) {
    ...
    for (i in urls) {
        ...
        routes.push(Route(parts[1], new RegExp('^'+(parts[3] || '')), context, urls[i][1]));
    }
...
clutch.route404 = function (urls, context, req, res) {
    ...
    var router = clutch.route(urls, context);
